...
# Cloudflare Auto Parking Script
# Ubuntu/Debian compatible
#
# Inputs:
#   1) Cloudflare API Token / API Key
...
print_header() {
  clear || true
  echo "============================================================"
  echo "  Cloudflare Auto Parking & Fixed DNS Setup"
  echo "============================================================"
  echo
}
...
verify_api_key() {
  echo "Verifying Cloudflare API token/key..."
  cf_api "GET" "/user/tokens/verify" >/dev/null
  echo "✅ API token/key verified."
  echo
}
...
get_zone_by_name() {
  ...
  cf_api "GET" "/zones?name=${encoded_domain}&per_page=50"
}
...
print_cloudflare_name_servers() {
  ...
  echo "Set these at your domain registrar:"
  echo "============================================================"
  ...
  payload="$(
    jq -n \
      ...
      '{
        ...
        ttl: 1,
        proxied: false,
        comment: "Created/updated by cloudflare-auto-parking-fixed.sh"
      }'
  )"
...
main() {
  ...
  echo
  echo "Important:"
  echo " - This script expects a Cloudflare API Token used as Bearer token."
  echo " - If you use the old Global API Key, Cloudflare normally also requires account email."
  echo

  CF_API_KEY="$(trim "${CF_API_KEY:-}")"

  if [[ -z "$CF_API_KEY" ]]; then
    echo "❌ API token/key cannot be empty."
    exit 1
  fi
